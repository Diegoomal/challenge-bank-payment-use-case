...
def test_credit_account_endpoint_completes_credit(tmp_path):
    database_path = str(tmp_path / "accounts.db")
    repository = SQLiteAccountRepository(database_path)
    repository.save(Account.create("merchant-1", "Merchant One", Decimal("100.00")))
...
def test_credit_account_endpoint_returns_failed_when_account_not_found(tmp_path):
    ...
    assert body["reason"] == "MERCHANT_ACCOUNT_NOT_FOUND"
