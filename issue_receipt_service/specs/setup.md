...
```bash
conda env create -n issue-receipt-service-env -f env.yml
conda activate issue-receipt-service-env
```
...
- `DATABASE_PATH`: SQLite database path. Default: `issue_receipt.db`.
...
```text
DATABASE_PATH=/data/issue_receipt.db
...
```bash
docker compose up -d --build issue_receipt_service issue_receipt_consumer issue_receipt_outbox
```

The root `docker-compose.yml` exposes this service on port `8007` and stores
the SQLite database in the `issue_receipt_data` volume.
...
3. Keep receipt issuing rules in the domain or application service.
