...
```bash
conda env create -n notify-merchant-service-env -f env.yml
conda activate notify-merchant-service-env
```
...
- `DATABASE_PATH`: SQLite database path. Default: `notify_merchant.db`.
...
```text
DATABASE_PATH=/data/notify_merchant.db
...
```bash
docker compose up -d --build notify_merchant_service notify_merchant_consumer notify_merchant_outbox
```

The root `docker-compose.yml` exposes this service on port `8005` and stores
the SQLite database in the `notify_merchant_data` volume.
...
3. Keep merchant notification rules in the domain or application service.
