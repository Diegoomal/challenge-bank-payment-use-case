...
def test_sqlite_notification_repository_gets_by_transaction_and_merchant(tmp_path):
    repository = SQLiteNotificationRepository(str(tmp_path / "notify.db"))
    notification = Notification.create_for_payment_confirmed(
        transaction_id="transaction-1",
        merchant_id="merchant-1",
    ...
    saved = repository.get_by_transaction_and_merchant("transaction-1", "merchant-1")
