...
```bash
conda env create -n notify-customer-service-env -f env.yml
conda activate notify-customer-service-env
```
...
- `DATABASE_PATH`: SQLite database path. Default: `notify_customer.db`.
...
```text
DATABASE_PATH=/data/notify_customer.db
...
```bash
docker compose up -d --build notify_customer_service notify_customer_consumer notify_customer_outbox
```

The root `docker-compose.yml` exposes this service on port `8006` and stores
the SQLite database in the `notify_customer_data` volume.
...
3. Keep customer notification rules in the domain or application service.
