...
This service exposes the FastAPI application on port `8001` when run directly:

```bash
PYTHONPATH=src uvicorn main:app --host 0.0.0.0 --port 8001
```

You can also run it manually with reload enabled:

```bash
PYTHONPATH=src uvicorn main:app --reload --port 8001
```
...
- `DATABASE_PATH`: SQLite database path. Default: `debit_account.db`.
...
```text
DATABASE_PATH=/data/debit_account.db
...
```bash
docker compose up -d --build debit_account_service debit_account_consumer debit_account_outbox
```

The root `docker-compose.yml` exposes this service on port `8001` and stores
the SQLite database in the `debit_account_data` volume.
...
3. Keep account debit rules in the domain or application service.
